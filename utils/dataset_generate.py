...
def SUM_RATE_GRAD(gs, schemes):
    ...
    """
    product = gs * schemes
    grad = gs / ((product + 1.0) * np.log(2))
    return grad

def alpha_calc(grad):
    grad_abs = np.abs(grad)
    sorted_indices = np.atleast_2d(np.argsort(-grad_abs[0]))
    for i in range(1, grad_abs.shape[0]):
        sorted_indices = np.concatenate((sorted_indices, np.atleast_2d(np.argsort(-grad_abs[i]))), axis=0)
    alpha = np.zeros_like(grad_abs, dtype=float)
    for i in range(sorted_indices.shape[0]):
        cur_sum = 0
        total = np.sum(grad_abs[i])
        sign = 0
        for j in range(sorted_indices.shape[1]):
            if sign == 1:
                alpha[i][sorted_indices[i][j]] = -1 if grad[i][sorted_indices[i][j]] > 0 else 1
                continue
            if (cur_sum + grad_abs[i][sorted_indices[i][j]]) >= (total / 2):
                alpha[i][sorted_indices[i][j]] = (total - grad_abs[i][sorted_indices[i][j]] - 2 * cur_sum) / grad_abs[i][sorted_indices[i][j]]
                alpha[i][sorted_indices[i][j]] *= 1 if grad[i][sorted_indices[i][j]] > 0 else -1
                sign = 1
                continue
            cur_sum = cur_sum + grad_abs[i][sorted_indices[i][j]]
            alpha[i][sorted_indices[i][j]] = 1 if grad[i][sorted_indices[i][j]] > 0 else -1
    return alpha

def SUM_RATE_GEN(sample_num=3, M=3, g_range=(0.5, 2.5), W=10.0):
    """
    Generate samples for the optimization problem of maximum sum rate of M channels.
    LRH-Gradient-Descent method.
    :param sample_num: number of samples to be generated.
    :param M: number of channels.
    :param g_range: range for generate channel gains vector g.
    :param W: total available power.
    :return: gs(sample_num, M), rates(sample_num), schemes(sample_num, M)
    """

    schemes = np.ones((sample_num, M)) * (W / M)
    gs = np.random.uniform(g_range[0], g_range[1], size=(sample_num, M))

    eps = 0.001
    beta = 0.1
    k = 1

    grad = SUM_RATE_GRAD(gs, schemes)
    while np.any(np.average(np.abs(grad), axis=1) > eps):
        grad = SUM_RATE_GRAD(gs, schemes)
        alpha = alpha_calc(grad)
        schemes = schemes + beta * alpha * grad
